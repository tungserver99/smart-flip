...
"$PYTHON_BIN" main.py flip_quantize   --model-path "$MODEL_PATH"   --models-root "$MODELS_ROOT"   --origin-method "$ORIGIN_METHOD"   --knee-tolerance "$KNEE_TOLERANCE"   --max-flip-percent "$MAX_FLIP_PERCENT"   "${EXTRA_ARGS[@]}"
