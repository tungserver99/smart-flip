...
Our goal is to keep the efficiency of AWQ while using a lightweight, statistics-guided rounding scheme
...
We keep the AWQ-style per-channel scaling, but replace pure nearest rounding with a **global greedy flipping**
procedure that uses activation statistics to decide which quantized integers should move by +1 or -1.

Key ingredients from the implementation (`awq_js_xl.py`):
- **L2 salience**: per-input-channel salience is `E[X^2]`.
...
The method is a James-Stein enhanced, heuristic AWQ variant:
- Per-channel scaling from L2 salience.
...
This keeps AWQ's efficiency while providing a more targeted control of quantization error than
nearest rounding.
