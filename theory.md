...
## 6. AWQ-style scaling như một xấp xỉ của variance term

Các phương pháp dựa trên salience và scaling, tiêu biểu như **AWQ**, thường sử dụng thống kê bậc hai dạng:

$$
\mathbb{E}[x_i^2]
$$

để đo mức độ “quan trọng” của từng input dimension.

Từ góc nhìn L2, điều này tương đương với việc xấp xỉ:

$$
\mathrm{Cov}(x)
\approx \mathrm{diag}\big(
\mathbb{E}[x_1^2], \dots, \mathbb{E}[x_d^2]
\big),
$$
...
$$
e^\top \mathrm{Cov}(x) e
\approx \sum_i \mathbb{E}[x_i^2] \, e_i^2.
$$

Scaling trước quantization trong AWQ có thể được hiểu là một cách **phân bổ sai số lượng tử hóa** sao cho các chiều có activation năng lượng lớn (salience cao) chịu sai số nhỏ hơn, từ đó giảm variance term của L2 loss dưới giả định covariance chéo không đáng kể.
...
## 8. Bức tranh tổng thể

Từ góc nhìn L2, ta có thể tóm tắt vai trò của các phương pháp như sau:

- **GPTQ**: tối ưu toàn bộ quadratic form  
  $$
  e^\top \mathbb{E}[x x^\top] e,
  $$
  kiểm soát cả bias và covariance, nhưng chi phí cao.

- **AWQ-style scaling**: xấp xỉ và giảm variance/covariance term thông qua salience $\mathbb{E}[x^2]$.

- **Phương pháp của chúng tôi**: trực tiếp kiểm soát bias term thông qua signed expectation  
  $$
  \mathbb{E}[x]^\top (w-w_q).
  $$

Phương pháp này mang tính **bổ sung (orthogonal)** với các phương pháp dựa trên salience/scaling, và có thể được kết hợp để đạt hiệu quả tốt hơn trong weight-only post-training quantization.
